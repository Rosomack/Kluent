...
abstract class AbstractJavaTimeComparator<T> where T : Comparable<T> {
    ...
    protected fun assertAtLeastAfter(theOther: T) {
        assertTrue(startValue >= theOther, "Expected $startValue to be at least { ${getExpectedOffset()} } after $theOther")
    }

    protected fun assertAtMostAfter(theOther: T) {
        assertTrue(startValue <= theOther, "Expected $startValue to be at most { ${getExpectedOffset()} } after $theOther")
    }

    protected fun assertExactlyAfter(theOther: T) {
        assertTrue(startValue == theOther, "Expected $startValue to be { ${getExpectedOffset()} } after $theOther")
    }

    protected fun assertExactlyBefore(theOther: T) {
        assertTrue(startValue == theOther, "Expected $startValue to be { ${getExpectedOffset()} } before $theOther")
    }

    protected fun assertAtLeastBefore(theOther: T) {
        assertTrue(startValue <= theOther, "Expected $startValue to be at least { ${getExpectedOffset()} } before $theOther")
    }

    protected fun assertAtMostBefore(theOther: T) {
        assertTrue(startValue >= theOther, "Expected $startValue to be at most { ${getExpectedOffset()} } before $theOther")
    }
